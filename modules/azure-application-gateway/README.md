...
| <a name="input_waf_allow_better_uptime_agent"></a> [waf\_allow\_better\_uptime\_agent](#input\_waf\_allow\_better\_uptime\_agent) | Configuration for allowing Better Uptime Agent on status URLs. Set to null to disable the rule. | <pre>object({<br/>    enabled     = bool<br/>    user_agent  = string<br/>    status_urls = list(string)<br/>  })</pre> | `null` | no |
