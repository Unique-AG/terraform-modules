...
`monitor_diagnostic_setting.log_analytics_destination_type` controls where Application Gateway resource logs land in Log Analytics.

- Defaults to `AzureDiagnostics` to preserve the destination used by previous module versions.
- Set it to `Dedicated` to route new Application Gateway logs to resource-specific tables: `AGWAccessLogs`, `AGWPerformanceLogs`, and `AGWFirewallLogs`.

Existing records stay in their original table after switching destination type. Query both old and new tables during the retention overlap.
...
| <a name="input_backend_5xx_alert"></a> [backend\_5xx\_alert](#input\_backend\_5xx\_alert) | Built-in alert on backend 5xx responses (BackendResponseStatus metric). excluded\_backend\_settings takes exact AGIC-generated BackendHttpSetting names to exclude (e.g. argocd). action\_group\_ids falls back to var.metric\_alerts\_external\_action\_group\_ids when empty. | <pre>object({<br/>    enabled                   = optional(bool, true)<br/>    threshold                 = optional(number, 100)<br/>    excluded_backend_settings = optional(list(string), [])<br/>    action_group_ids          = optional(list(string), [])<br/>  })</pre> | `{}` | no |
