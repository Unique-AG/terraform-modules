locals {
  maintenance_window_validations = {
    valid_frequencies  = ["Daily", "Weekly", "AbsoluteMonthly", "RelativeMonthly"]
    valid_days_of_week = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    valid_week_indexes = ["First", "Second", "Third", "Fourth", "Last"]
...
resource "azurerm_kubernetes_cluster" "cluster" {
  ...
  dynamic "maintenance_window_auto_upgrade" {
    ...
    content {
      ...
      dynamic "not_allowed" {
        for_each = var.maintenance_window_auto_upgrade.not_allowed
        content {
  ...
  dynamic "auto_scaler_profile" {
    ...
    content {
      max_graceful_termination_sec     = var.node_autoscaling.cluster_autoscaler.max_graceful_termination_sec
      skip_nodes_with_local_storage    = var.node_autoscaling.cluster_autoscaler.skip_nodes_with_local_storage
      expander                         = var.node_autoscaling.cluster_autoscaler.expander
      scale_down_unneeded              = var.node_autoscaling.cluster_autoscaler.scale_down_unneeded
      scale_down_delay_after_delete    = var.node_autoscaling.cluster_autoscaler.scale_down_delay_after_delete
      scale_down_utilization_threshold = var.node_autoscaling.cluster_autoscaler.scale_down_utilization_threshold
    }
  }
