...
- **aks_unschedulable_pods**: Metric alert for unschedulable pods (severity 2), signaling an issue with cluster autoscaling which can mean that the pools maximum size is reached
...
| [azurerm_monitor_metric_alert.nodepool_at_max_capacity](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/monitor_metric_alert) | resource |
...
| <a name="input_alerts"></a> [alerts](#input\_alerts) | Map of alerts to create for the AKS cluster. Supports activity log alerts, metric alerts, and auto-generated nodepool capacity alerts. Set to {} to disable all default alerts. | <pre>map(object({<br/>    name        = optional(string)<br/>    description = optional(string, "")<br/>    enabled     = optional(bool, true)<br/>    severity    = optional(number, 3)<br/>    frequency   = optional(string, "PT5M")<br/>    window_size = optional(string, "PT15M")<br/><br/>    # Activity log alert criteria<br/>    activity_log_criteria = optional(object({<br/>      operation_name = string<br/>      category       = string<br/>      levels         = optional(list(string), ["Error"])<br/>      statuses       = optional(list(string), ["Failed"])<br/>    }))<br/><br/>    # Metric alert criteria<br/>    metric_criteria = optional(object({<br/>      metric_namespace       = optional(string, "Microsoft.ContainerService/managedClusters")<br/>      metric_name            = string<br/>      aggregation            = string<br/>      operator               = string<br/>      threshold              = number<br/>      skip_metric_validation = optional(bool, false)<br/>      dimension = optional(list(object({<br/>        name     = string<br/>        operator = string # Include, Exclude, StartsWith<br/>        values   = list(string)<br/>      })), [])<br/>    }))<br/><br/>    # Nodepool capacity alert criteria - auto-generates one alert per nodepool<br/>    # Uses kube_node_status_condition metric filtered by node name pattern<br/>    nodepool_capacity_criteria = optional(object({}))<br/><br/>    actions = optional(list(object({<br/>      action_group_id    = string<br/>      webhook_properties = optional(map(string), {})<br/>    })))<br/>  }))</pre> | <pre>{<br/>  "aks_agent_pool_write_error": {<br/>    "activity_log_criteria": {<br/>      "category": "Administrative",<br/>      "levels": [<br/>        "Error"<br/>      ],<br/>      "operation_name": "Microsoft.ContainerService/managedClusters/agentpools/write",<br/>      "statuses": [<br/>        "Failed"<br/>      ]<br/>    },<br/>    "description": "Alerts when agent pool write operations fail",<br/>    "name": "AKS Agent Pool Write Error"<br/>  },<br/>  "nodepool_at_max_capacity": {<br/>    "description": "Alerts when a nodepool is running at maximum node count",<br/>    "name": "Nodepool At Max Capacity",<br/>    "nodepool_capacity_criteria": {},<br/>    "severity": 2,<br/>    "window_size": "PT30M"<br/>  }<br/>}</pre> | no |
