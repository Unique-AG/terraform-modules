...
| <a name="input_upgrade_override"></a> [upgrade\_override](#input\_upgrade\_override) | Override settings for AKS cluster upgrades. Forces upgrades past safeguards. Once set, this block cannot be removed from the configuration. | <pre>object({<br/>    force_upgrade_enabled = bool<br/>    effective_until       = optional(string)<br/>  })</pre> | `null` | no |
