...
Both are dynamic parameters, so they take effect without a server restart. They are not required for the server to run — if you override `parameter_values`, you may safely omit them; the server will still work.
...
| [azurerm_management_lock.can_not_delete_server](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/management_lock) | resource |
...
No change in is breaking ✅

- `storage_mb` is now `lifecycle.ignore_changes` users of the module that don't have auto-grown enabled will need to manually update their configuration once their server grew its storage.
- `metric_alerts_external_action_group_ids` must now be provided. Users not wanting alerts must explicitly `[]` empty the list (`metric_alerts_external_action_group_ids = []`).