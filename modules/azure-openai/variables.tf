...
variable "cognitive_accounts" {
  description = <<-EOT
    ...
    `log_categories` locks the exact list. Valid values for OpenAI: Audit, AzureOpenAIRequestUsage, RequestResponse, Trace.
  EOT
  type = map(object({
...
variable "diagnostic_settings" {
  description = <<-EOT
    ...
    Available log categories (when using explicit `log_categories` and `log_category_groups` is empty):
      ...
      - AzureOpenAIRequestUsage: Token usage and request metering for OpenAI deployments
